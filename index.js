...
var debug = require('debug')('ddrive-http')
...
function ondirectoryindex (archive, name, req, res, opts) {
  list(archive, name, function (err, entries) {
    ...
    if (opts.exposeHeaders) {
      res.setHeader('DDrive-Key', archive.key.toString('hex'))
      res.setHeader('DDrive-Version', archive.version)
...
function getManifest (archive, cb) {
  archive.readFile('/dwebx.json', 'utf-8', function (err, data) {
    ...
    if (!parsed || Array.isArray(parsed) || (typeof parsed !== 'object')) {
      return cb(new Error('Invalid dwebx.json format'))
    }
